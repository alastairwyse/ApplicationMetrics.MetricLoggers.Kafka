...
namespace ApplicationMetrics.MetricLoggers.Kafka
{
    ...
    public class KafkaMetricConsumer : IDisposable
    {
        ...
        /// <param name="kafkaErrorHandlingAction">An action to invoke if the underlying Kafka <see cref="IConsumer{TKey, TValue}"/> raises a Kafka <see cref="Error"/> when a metric is consumed from the cluster.  Accepts a single parameter which is the <see cref="Error"/>.</param>
        ...
        /// <remarks>
        ///   <para>According to the <see href="https://docs.confluent.io/platform/current/clients/confluent-kafka-dotnet/_site/api/Confluent.Kafka.ConsumerBuilder-2.html">documentation</see> for the <see cref="ConsumerBuilder{TKey, TValue}.SetErrorHandler(Action{IConsumer{TKey, TValue}, Error})"/> and <see cref="ConsumerBuilder{TKey, TValue}.SetLogHandler(Action{IConsumer{TKey, TValue}, LogMessage})"/> which are used to invoke the <paramref name="kafkaErrorHandlingAction"/> and <paramref name="logMessageAction"/> parameters, exceptions thrown in these actions will be silently ignored.  Hence exceptions thrown in these parameters cannot be caught and acted on by client code.</para>
        ///   <para>The Kafka <see cref="IConsumer{TKey, TValue}"/> is resilient to network errors when connecting to the cluster, and will raise Kafka <see cref="Error">Errors</see> which will be passed to parameter <paramref name="kafkaErrorHandlingAction"/> in these cases, and consumption attempts will continue.  However for more fatal errors like the specified topic not existing in the cluster , the <see cref="IConsumer{TKey, TValue}"/> will throw an <see cref="Exception"/> which will be passed to parameter <paramref name="consumeExceptionAction"/>, the worker thread performing the consumption stopped, and the exception rethrown on the main thread when the <see cref="KafkaMetricConsumer.Stop"/> method is called.</para>
        /// </remarks>
