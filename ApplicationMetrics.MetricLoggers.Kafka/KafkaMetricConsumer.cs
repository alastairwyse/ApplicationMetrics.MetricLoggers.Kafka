...
namespace ApplicationMetrics.MetricLoggers.Kafka
{
    ...
    public class KafkaMetricConsumer : IDisposable
    {
        ...
        ///   <para>The Kafka <see cref="IConsumer{TKey, TValue}"/> is resilient to network errors when connecting to the cluster, and will raise Kafka <see cref="Error">Errors</see> which will be passed to parameter <paramref name="kafkaErrorHandlingAction"/> in these cases, and consumption attempts will continue.  However for more fatal errors like the specified topic not existing in the cluster, the associated <see cref="Exception"/> will be passed to parameter <paramref name="consumeExceptionAction"/>, the worker thread performing the consumption stopped, and the exception rethrown on the main thread when the <see cref="KafkaMetricConsumer.Stop"/> method is called.</para>
        /// </remarks>
