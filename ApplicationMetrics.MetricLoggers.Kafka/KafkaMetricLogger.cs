...
namespace ApplicationMetrics.MetricLoggers.Kafka
{
    ...
    public class KafkaMetricLogger : MetricLoggerBuffer
    {
        ...
        /// <param name="kafkaErrorHandlingAction">An action to invoke if the underlying Kafka <see cref="IProducer{TKey, TValue}"/> raises a Kafka <see cref="Error"/> when a metric is written to the cluster.  Accepts a single parameter which is the <see cref="Error"/>.</param>
